...
fluxbias = GS200("source", 'TCPIP0::169.254.0.14::inst0::INSTR', terminator="\n")
...
fr_range = np.arange(200e6, 242e6, 0.05e6) # IF Hz
...
fig, axs = plt.subplots(nrows=2, ncols=8, layout='constrained', figsize=(16, 7))
...
try:
    for i in range(Ngate):
        ...
        #######
        # estimate qubit freq and set LO
        ...
        # Set LO to IF in range +0 to +100MHz
        ...
        # Set qubit LO such that IF in range -200 to -300 MHz
        qubitLO = int(max(2e9, np.ceil((fqest+0.10e9)/1e8)*1e8))
        print(f"  Choose qubit LO at {qubitLO/1e9}GHz")
        localconfig['qubitIF'] = fqest - qubitLO
        localconfig['qubitLO'] = qubitLO
