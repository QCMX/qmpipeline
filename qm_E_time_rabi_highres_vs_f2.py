...
actual_overlap = drive_read_overlap # due to additional short waveform
...
with qua.program() as time_rabi:
    ...
    with qua.stream_processing():
        n_st.save('iteration')
        I_st.buffer(len(f2), len(duration_clk)*4+16).average().save('I')
        Q_st.buffer(len(f2), len(duration_clk)*4+16).average().save('Q')
...
fig, ax = plt.subplots()
img = plt2dimg(ax, duration_ns, f2/1e6, np.full((duration_ns.size, f2.size), np.nan))
fig.colorbar(img, ax=ax).set_label('arg S')
ax.set_xlabel('drive duration / ns')
ax.set_ylabel('qubit drive IF / MHz')
...
np.savez_compressed(
    ...
    duration_clk=duration_clk, duration_ns=duration_ns, f2=f2,
...
fig.suptitle(
    ...
    f"   qubit LO {config.qubitLO/1e9:f}GHz"
    f"   Navg {Nactual}"
