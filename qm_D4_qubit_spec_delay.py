...
delay_min = 0 # cycles
delay_max = 2000//4 # cycles
delay_step = 400//4 # cycles
delays = int_forloop_values(delay_min, delay_max, delay_step)
delays_ns = delays * 4

saturationdelay = 500//4 #cycles
...
with qua.program() as qubit_delay:
    ...
    with qua.stream_processing():
        n_st.save('iteration')
        I_st.buffer(len(freqs), len(delays)).average().save('I')
        Q_st.buffer(len(freqs), len(delays)).average().save('Q')
...
try:
    while res_handles.is_processing():
        ...
        plt2dimg_update(img, np.unwrap(np.unwrap(np.angle(Z), axis=1), axis=0))
