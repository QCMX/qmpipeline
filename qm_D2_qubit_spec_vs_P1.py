...
try:
    while res_handles.is_processing():    
        ...
        for i in range(len(amps)):
            artists.append(axs.plot(freqs, np.abs(Z[i])-np.mean(np.abs(Z[i])), color=plt.cm.rainbow(i/len(amps)))[0])
        axs.relim()
        axs.autoscale()
        axs.autoscale_view()
...
np.savez_compressed(
    fpath, Navg=Navg, freqs=freqs, Zraw=Zraw, amps=amps, ampsV=ampsV,
...
for i in range(len(amps)):
    c = colors[i]
    axs[0,0].plot(freqs/1e6, magZ[i]-np.mean(magZ[i]), color=c)
