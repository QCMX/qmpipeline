...
with qua.program() as tof_cal:
    ...
    #update_frequency('vna', 100e6)

    with qua.for_(n, 0, n < Navg, n + 1):
        qua.reset_phase('vna') # reset the phase of the next played pulse
        #qua.play('preload', 'vna')
        #qua.play('saturation', 'qubit')
        qua.measure('readout', 'vna', adc_st)
        qua.wait(config.cooldown_clk, 'vna')  # wait for photons in vna to decay
        qua.save(n, n_st)
...
if simulate:
    ...
    fig.suptitle(
        f"LO={config.vnaLO/1e9:.5f}GHz   IF={config.vnaIF/1e6:.3f}MHz"
        ...
        f"\n{config.qmconfig['elements']['vna']['smearing']}ns smearing", fontsize=10)
    fig.tight_layout()
...
p0 = [config.vnaIF/1e6, 0.5, 250/1e3, 50, np.mean(adc1), 0]
