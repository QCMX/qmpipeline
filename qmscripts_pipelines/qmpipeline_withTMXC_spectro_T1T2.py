...
Vgate = np.linspace(-4.826, -4.820, 16)
Vgate = np.array([-4.8208])
...
fr_range = np.arange(190e6, 225e6, 0.05e6) # IF Hz
...
results = {
    ...
    'qubit_P2': [None]*Ngate,
    'resonator_qubitP2': [None]*Ngate,
    'spectro_f2_P1:1': [None]*Ngate,
    'spectro_f2_P1:2': [None]*Ngate,
...
fig, axs = plt.subplots(nrows=2, ncols=8, layout='constrained', figsize=(17, 7))
frline, = axs[0,0].plot(Vgate, np.full(len(Vgate), np.nan), '.-', linewidth=1)
axs[0,0].set_xlabel('Vgate / V')
axs[0,0].set_ylabel('df / MHz')
axs[0,0].set_title(f"Cavity resonance vs gate\ncooldown: {baseconfig['cooldown_clk']*4/1e3:.0f}us", fontsize=8)
...
try:
    for i in range(Ngate):
        ...
        prog = progs[2] = qmtools.QMResonatorSpec_P2(
            qmm, localconfig, Navg=600,
            resonatorIFs=fr_range,
            readout_amps=np.logspace(-60/20, -10/20, 11)*AMP0dBm)
        ...
        fq_amps = np.logspace(-50/20, -10/20, 9) * AMP0dBm
        assert np.max(fq_amps) <= 0.32 # max 0dBm
        prog = progs[3] = qmtools.QMQubitSpec_P2(
            qmm, localconfig, Navg=2000, qubitIFs=np.arange(-450e6, +450e6, 2e6),
            drive_amps=fq_amps)
        results['qubit_P2'][i] = qubitspecvspower = prog.run(plot=axs[0,2])

        # Move qubit IF to -100 to -200 MHz
        qubitLO = int(max(2e9, np.ceil((fqest+0.10e9)/1e8)*1e8))
        ...
        # For large frequencies have big fqest error, so fine tune again after adjusting LO
        # Adjusts LO to -150 to -250MHz
        if fqest >= 3.85e9:
        ...
        fq = localconfig['qubitIF'] + localconfig['qubitLO']

        #######
        ## Qubit respons to cavity photons
        ...
        # choose qubit power
        localconfig['qubit_output_gain'] = -10
        localconfig['saturation_amp'] = 10**(-20/20)*AMP0dBm if fq < 3.8e9 else 10**(-40/20)*AMP0dBm

        qubitIFs = np.arange(-100e6, 100e6, 1e6) + max(np.round(localconfig['qubitIF']/10e6)*10e6, -350e6)
        prog = progs[19] = qmtools.QMQubitSpec_P1(
            qmm, localconfig, Navg=1000,
            qubitIFs=qubitIFs,
            readout_amps=np.logspace(-60/20, -30/20, 21)*AMP0dBm)
        results['spectro_f2_P1:1'][i] = res = prog.run(plot=axs[0,7])

        # choose lower qubit power
        localconfig['qubit_output_gain'] = -10
        localconfig['saturation_amp'] = 10**(-5/20) * (10**(-20/20)*AMP0dBm if fq < 3.8e9 else 10**(-40/20)*AMP0dBm)

        qubitIFs = np.arange(-100e6, 100e6, 1e6) + max(np.round(localconfig['qubitIF']/10e6)*10e6, -350e6)
        prog = progs[20] = qmtools.QMQubitSpec_P1(
            qmm, localconfig, Navg=1000,
            qubitIFs=qubitIFs,
            readout_amps=np.logspace(-60/20, -30/20, 21)*AMP0dBm)
        results['spectro_f2_P1:2'][i] = res = prog.run(plot=axs[1,7])

        localconfig['qmconfig']['waveforms']['readout_wf']['sample'] = baseconfig['qmconfig']['waveforms']['readout_wf']['sample']

        #######
        ## Cavity response to drive qubit

        #localconfig['qubit_output_gain'] = -10
        localconfig['resonator_output_gain'] = -20
        localconfig['readout_amp'] = 0.001
        prog = progs[18] = qmtools.QMResonatorDriveQubit(
            qmm, localconfig, Navg=800,
            resonatorIFs=fr_range, drive_amps=np.logspace(-60/20, -0/20, 11)*AMP0dBm)
        results['resonator_qubitP2'][i] = res = prog.run(plot=axs[0,6])
        ...
        ## VS P1 f1
        prog = progs[6] = qmtools.QMReadoutSNR(
            qmm, localconfig, Navg=3e3,
            resonatorIFs=np.arange(195e6, 220e6, 0.5e6),
            readout_amps=np.logspace(-60/20, 0, 31) * AMP0dBm,
            drive_len=1500)
        results['readoutSNR'][i] = res = prog.run(plot=axs[1,6])
        ...
        prog = progs[5] = qmtools.QMReadoutSNR_P1(
            ...
            drive_len=1000)
        results['readoutSNR_P1:non-demolition'][i] = res = prog.run(plot=axs[1,2])
        ...
        #######
        # Time Rabi Chevrons
        localconfig['saturation_amp'] = 0.316 if fq < 3.8e9 else 0.1
        ...
        prog = progs[7] = qmtools.QMTimeRabiChevrons(
            ...
            qubitIFs=np.arange(-450e6, -50e6, 5e6),
            max_duration_ns=PROTOCOL_DURATION,
            drive_read_overlap_cycles=0)
        ...
        #     qmm, dispersiveconfig, Navg=5000, resonatorIFs=fr_range, drive_len_ns=32, sigma_ns=8)
        # results['resonator_excited:16ns'][i] = prog.run(plot=axs[1,0])
        ...
        prog = progs[13] = qmtools.QMRamseyChevronRepeat_Gaussian(
            ...
            max_delay_ns=PROTOCOL_DURATION)
        results['ramsey_chevron_repeat:gaussian_8ns'][i] = prog.run(plot=axs[0,5])
        
        #######
        # Hahn Echo 8ns
        fc = np.round(localconfig['qubitIF']/10e6)*10e6
        ifs = np.arange(-20e6, 20e6, 2e6) + fc #np.linspace(-80e6, 80e6, 17) + fc
        prog = progs[16] = qmtools.QMHahn(
            qmm, localconfig, Nrep=2, Navg=80, qubitIFs=ifs,
            max_delay_ns=PROTOCOL_DURATION//2, drive_len_ns=16, sigma_ns=4, readout_delay_ns=8)
        results['hahn_echo:8ns'][i] = prog.run(plot=axs[1,5])
    ...
    np.savez_compressed(
        ...
        resonator=resonator, resonatorfit=frfit, temperature=temp,
        jpameta=jpameta,
