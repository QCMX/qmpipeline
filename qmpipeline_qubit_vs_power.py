...
fq_range = np.arange(-400e6, +100e6, 2e6)
fq_amps = np.logspace(np.log10(0.01), np.log10(0.1), 21)
...
time_rabi_IFs = np.arange(-350e6, -150e6, 20e6)
time_rabi_max_duration = 60 # ns
...
fig, axs = plt.subplots(nrows=2, ncols=4, layout='constrained', figsize=(12, 7))
...
try:
    for i in range(Ngate):
        ...
        localconfig['readout_amp'] = baseconfig['readout_amp'] # restore
        ...
        prog = progs[2] = qmtools.QMQubitSpec_P2(
            qmm, localconfig, Navg=700, qubitIFs=fq_range, drive_amps=fq_amps)
        ...
        # # axs[0,1].axhline(
        # #     qmtools.opx_amp2pow(localconfig['short_readout_amp'], localconfig['resonator_output_gain']),
        # #     color='fuchsia', linewidth=0.8, zorder=100)
        # prog = progs[1] = qmtools.QMReadoutSNR(
        #     qmm, localconfig, Navg=6e5,
        #     resonatorIFs=np.arange(200e6, 212e6, 2e6),
        #     readout_amps=np.logspace(np.log10(0.0001), np.log10(0.316), 20),
        #     drive_len=200)
        # results['readoutSNR'][i] = prog.run(plot=axs[0,1])

        # localconfig['saturation_amp'] = 0.316
        localconfig['qubit_output_gain'] = +10 #baseconfig['qubit_output_gain']
        # localconfig['short_readout_amp'] = 0.177 # -5 dBm
        localconfig['resonator_output_gain'] = -20 # baseconfig['resonator_output_gain'] # restore
        ...
        prog = progs[5] = qmtools.QMPowerRabi(
            qmm, localconfig, Navg=1e6, duration_ns=8,
