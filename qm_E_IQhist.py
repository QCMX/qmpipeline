...
line, = ax[0].plot(freqs/1e6, np.full(len(freqs), np.nan), label="|Z|")
ax[0].set_title('resonator spectroscopy analysis')
ax[0].set_xlabel('IF [MHz]')
ax[0].set_ylabel('demod signal [a.u.]')
iqdata, = ax[1].plot(I_handle.fetch_all(), I_handle.fetch_all(), ls="", marker=".")
fig.show()
try:
    while res_handles.is_processing():
        ...
        ax[0].relim(), ax[0].autoscale(), ax[0].autoscale_view()
        iqdata.set_data(I,Q)
        ax[1].relim(), ax[1].autoscale(), ax[1].autoscale_view()
        print(iteration)
        mpl_pause(0.5)
